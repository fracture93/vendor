...
function Addon:DumpLink_Cmd(arg)
    Addon:Output(Addon.Systems.Chat.MessageType.Console, "Link: "..tostring(arg))
    Addon:Output(Addon.Systems.Chat.MessageType.Console, "Raw: "..gsub(arg, "\124", "\124\124"))
    Addon:Output(Addon.Systems.Chat.MessageType.Console, "ItemString: "..tostring(self:GetLinkFromString(arg)))
    local props = self:GetLinkPropertiesFromString(arg)
    for i, v in pairs(props) do
        Addon:Output(Addon.Systems.Chat.MessageType.Console, "["..tostring(i).."] "..tostring(v))
    end
    Addon:Output(Addon.Systems.Chat.MessageType.Console, "ItemInfo:")
...
function Addon:ToggleSimulate_Cmd()
    ...
    Addon:Output(Addon.Systems.Chat.MessageType.Console, "Simulate set to %s", tostring(Addon:GetDebugSetting("simulate")))
end

function Addon:ClearCache_Cmd()
    Addon:ClearItemResultCache("Debug Command")
    Addon:Output(Addon.Systems.Chat.MessageType.Console, "Item cache and result cache cleared.")
end
...
function Addon:Profile_Cmd(cmd, arg1, arg2)
    ...
    if not cmd then
        ...
        Addon:Output(Addon.Systems.Chat.MessageType.Console, "All available profiles:")
        for _, profile in pairs(profiles) do
            Addon:Output(Addon.Systems.Chat.MessageType.Console, "  %s [%s]", tostring(profile:GetName()), tostring(profile:GetId()))
        end
        Addon:Output(Addon.Systems.Chat.MessageType.Console, "Active profile: %s", tostring(active:GetName()))
    ...
    elseif cmd == "set" and type(arg1) == "string" then
        if not Addon:ProfileExists(arg1) then
            Addon:Output(Addon.Systems.Chat.MessageType.Console, "Profile '%s' does not exist.", arg1)
            return
        end
        if Addon:SetProfile(arg1) then
            Addon:Output(Addon.Systems.Chat.MessageType.Console, "Active profile set to '%s'", arg1)
        else
            Addon:Output(Addon.Systems.Chat.MessageType.Console, "Failed setting active profile.")
        end
    ...
    elseif cmd == "copy" and type(arg1) == "string" and type(arg2) == "string" then
        ...
        if Addon:ProfileExists(arg2) then
            Addon:Output(Addon.Systems.Chat.MessageType.Console, "Profile '%s' already exists.", arg2)
            return 
        end
        if Addon:CopyProfile(arg1, arg2) then
            Addon:Output(Addon.Systems.Chat.MessageType.Console, "Profile '%s' copied as profile '%s'", arg1, arg2)
        else
            Addon:Output(Addon.Systems.Chat.MessageType.Console, "Failed copying the profile.")
        end
    ...
    elseif cmd == "delete" and type(arg1) == "string" then
        ...
        if Addon:DeleteProfile(arg1) then
            Addon:Output(Addon.Systems.Chat.MessageType.Console, "Profile '%s' has been deleted.", arg1)
        else
            Addon:Output(Addon.Systems.Chat.MessageType.Console, "Failed deleting the profile.")
        end

    elseif cmd == "create" and type(arg1) == "string" then
        if Addon:ProfileExists(arg1) then
            Addon:Output(Addon.Systems.Chat.MessageType.Console, "Profile '%s' already exists.", arg1)
            return
        end
        if Addon:NewProfile(arg1) then
            Addon:Output(Addon.Systems.Chat.MessageType.Console, "Profile '%s' has been created and set to the active profile.", arg1)
        else
            Addon:Output(Addon.Systems.Chat.MessageType.Console, "Failed creating the profile.")
        end
    ...
    elseif cmd == "rename" and type(arg1) == "string" and type(arg2) == "string" then
        ...
        if Addon:RenameProfile(arg1, arg2) then
            Addon:Output(Addon.Systems.Chat.MessageType.Console, "Profile '%s' has been renamed to '%s'.", arg1, arg2)
        else
            Addon:Output(Addon.Systems.Chat.MessageType.Console, "Failed renaming the profile.")
        end
    else
        Addon:Output(Addon.Systems.Chat.MessageType.Console, "profile usage: <none> | get | set | create | copy | delete | rename")
    end
end
...
function Addon:DumpTooltipItemProperties()
    ...
    for i, v in Addon.orderedPairs(props) do
        if not string.find(i, "^Is") then
            ...
            Addon:Output(Addon.Systems.Chat.MessageType.Console, "    [%s] %s", tostring(i), tostring(val))
        end
    end
